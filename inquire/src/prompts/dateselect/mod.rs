...
impl<'a> DateSelect<'a> {
    ...
    /// Async variant of [`prompt_skippable`](Self::prompt_skippable) for the `no-tty` feature.
    #[cfg(feature = "no-tty")]
    pub async fn prompt_skippable(
        self,
        event: crossterm::event::NoTtyEvent,
        sender: crossterm::event::SenderWriter,
    ) -> InquireResult<Option<NaiveDate>> {
    ...
    pub(crate) async fn prompt_with_backend<B: DateSelectBackend>(
        ...
        DateSelectPrompt::new(self)?.prompt(backend).await
    }
}
